...
# Asynchronous functions to convert text to an audio file
async def TextToAudioFile(text) -> None:
    file_path = r"Data/speech.mp3" # Define the path where the speech file will be saved

    if os.path.exists(file_path):  # Check if the file already exists
        os.remove(file_path)  # If is exists, remove it to avoid overwriting errors

    # Create the communicate object to generate speech
    communicate = edge_tts.Communicate(text, AssistantVoice,pitch='+3Hz' , rate='+20%')
        ...
        try:
            ...
            try:
                ...
                if pygame.mixer.get_init():
                    ...
                    pygame.mixer.quit()  # Quit the pygame mixer to free resources

                # Small delay to ensure audio resources are fully released
                import time
                time.sleep(0.1)
...
# Function to manage Text-to-Speech (TTS) functionality with additional responses for long text
